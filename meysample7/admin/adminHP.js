    ...
    // Verify admin password (AJAX, not hardcoded)
    async function verifyPassword() {
        const password = document.getElementById('adminPassword').value;
        try {
            const response = await fetch('verify_admin_password.php', {
                method: 'POST',
            ...
            Swal.fire({
                title: 'Error',
                text: 'Could not verify password. Please try again.',
                icon: 'error',
                confirmButtonColor: '#036d2b'
            });
        }
...
async function loadDashboardStats() {
    try {
        ...
        if (data.success) {
            ...
            document.querySelector('.stat-card:nth-child(1) h3').textContent = data.data.activeUsers;
            document.querySelector('.stat-card:nth-child(2) h3').textContent = data.data.books;
            document.querySelector('.stat-card:nth-child(3) h3').textContent = data.data.overdue;
        } else {
    ...
    async function filterByUserStatus(userType) {
        try {
            ...
            const searchTerm = document.getElementById(`${userType}Search`).value.trim();
            
            let url = `fetch_${userType}.php?`;
            ...
            if (searchTerm) {
                url += `${statusFilter !== 'all' ? '&' : ''}search=${encodeURIComponent(searchTerm)}`;
            }
            ...
            if (data.data.length === 0) {
                const row = document.createElement('tr');
                row.innerHTML = `<td colspan="5" class="no-results">No users found matching your criteria</td>`;
                tableBody.appendChild(row);
                return;
            }
    ...
    'inventory': `
        ...
        <div class="section-header">
            ...
            <div class="tab-buttons">
                <button onclick="showTab('contact')" class="tab-btn active"><i class="fas fa-address-book"></i> Contact Info</button>
            ...
            <!-- Contact Info Tab -->
            <div class="tab-content" id="contact" style="display: block;">
    ...
    // --- Backup Date Input Type Handler ---
    function attachBackupDateInputHandler() {
        const filterSelect = document.getElementById('backupFilter');
        const dateContainer = document.getElementById('dateRangeContainer');
        const dateInput = document.getElementById('backupDateRange');

        if (!filterSelect || !dateContainer || !dateInput) return;
        ...
        function updateDateInput() {
            ...
            if (filter === 'all') {
                ...
                dateInput.type = 'date';
                dateInput.value = '';
                dateInput.placeholder = 'Select day';
            } else if (filter === 'month') {
                dateContainer.style.display = '';
                dateInput.type = 'month';
                dateInput.value = '';
                dateInput.placeholder = 'Select month';
            } else if (filter === 'year') {
                dateContainer.style.display = '';
                ...
                dateInput.value = '';
                dateInput.placeholder = 'Select year';
            }
        }

        filterSelect.addEventListener('change', updateDateInput);
        updateDateInput();
    }