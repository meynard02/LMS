...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Check admin table (admins don't need OTP)
    $adminStmt = $conn->prepare("SELECT AdminID AS id, AdminPassword AS password, AdminFName, AdminLName FROM admin WHERE AdminUsername = ?");
    ...
    // Check user table
    $userStmt = $conn->prepare("SELECT UserID AS id, Password AS password, FirstName, Email FROM user WHERE (Email = ? OR FirstName = ?) AND `is_verified` = 1");
